...
const AppHeader: React.FC = () => {
  ...
  const handleQuickRoleChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newRole = e.target.value as UserRole;
    switchRole(newRole);
    if (newRole === "SMC_MEMBER") navigate("/dashboard/smc");
    else if (newRole === "CITIZEN") navigate("/dashboard/citizen");
    else if (newRole === "GOVERNMENT_OFFICER") navigate("/dashboard/authority");
    else if (newRole === "SAMARTHYA_ADMIN") navigate("/dashboard/admin");
  };
      ...
      <div className="flex grow flex-col items-center justify-between xl:flex-row xl:px-6">
        ...
        >
          <div className="flex items-center gap-2 2xsm:gap-3">
            ...
            <div className="flex items-center gap-1.5 rounded-xl border border-brand-200 bg-brand-50/70 px-2.5 py-1.5 dark:border-brand-900/50 dark:bg-brand-950/30">
              <span className="text-xs font-semibold text-brand-700 dark:text-brand-300 hidden sm:inline">Role:</span>
              <select
                value={role}
                onChange={handleQuickRoleChange}
                aria-label="Select User Role"
                className="bg-transparent text-xs font-bold text-brand-900 dark:text-brand-200 focus:outline-hidden cursor-pointer"
              >
                <option value="SMC_MEMBER" className="bg-white text-gray-900 dark:bg-gray-900 dark:text-white">🏫 SMC Member</option>
                <option value="CITIZEN" className="bg-white text-gray-900 dark:bg-gray-900 dark:text-white">👥 Public Citizen</option>
                <option value="GOVERNMENT_OFFICER" className="bg-white text-gray-900 dark:bg-gray-900 dark:text-white">🏛️ Govt Authority</option>
